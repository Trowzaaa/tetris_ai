...
from tetris_model import BOARD_DATA, SHAPE, BoardData, Shape
import tetris_ai
...
class Tetris(QMainWindow):
    ...
    def timerEvent(self, event):
        if event.timerId() == self.timer.timerId():
            if ai_activated and not self.nextMove:
                self.nextMove = tetris_ai.TETRIS_AI.nextMove()
            if self.nextMove:
                k = 0
            ...
            if self.tboard.score < 250:
                pass
            else:
                print(BoardData.Self)
